...
flowchart TD
    ...
    %% STANDARDNÍ NABÍDKA (MARKETPLACE)
    A --> B[Marketplace – Tikety (standardní nabídka)]
    B --> B1[Tiket detail<br/>+ Projekt část (maskovaný/odemknutý)]
    B1 --> B2[Vytvořit rezervaci (stepper)]

    %% REZERVACE
    A --> C[Rezervace]
    C --> C1[Rezervace detail<br/>+ SLA timeline]

    %% INVESTOŘI (INTERNÍ EVIDENCE)
    A --> D[Investoři (evidence)]
    D --> D1[Investor detail]
    D --> D2[Nový investor / editace]
    D1 --> E[Matching – doporučení investorů]
    B2 --> E

    %% BROKER LEADY (PROJEKTY PŘINESENÉ BROKEREM)
    A --> F[Projekty (lead od brokera)]
    F --> F1[Lead/Projekt detail<br/>+ stav + přiřazení developera]
    F1 --> F2[Tikety v projektu (po zveřejnění)]
    F2 --> B1

    %% MONEY & SETTINGS
    A --> G[Provize a výplaty]
    G --> G1[Provize detail / fakturace]
...
flowchart TD
    ...
    T2 --> R1[Rezervace detail + rozhodnutí]
    R1 --> F1[Potvrdit financování (form)]

    A --> R[Rezervace (napříč tikety)]
    R --> R1

    A --> F[Poplatky / provize platformě]
    A --> S[Nastavení profilu]
    A --> H[Podpora]
```
...
flowchart TD
    ...
    U --> U1[Brokeři]
    U --> U2[Developeri]
    U1 --> U1d[Detail brokera + schválení]
    U2 --> U2d[Detail developera + schválení]

    A --> P[Projekty]
    P --> P1[Projekt detail + schválení/publikace]
    P1 --> T[Tikety v projektu]
    T --> T1[Tiket detail + nastavení kapacit/SLA]

    A --> R[Rezervace]
    R --> R1[Rezervace detail + zásahy]

    A --> M[Platby & provize]
    M --> M1[Evidence plateb developer→platforma]
    M --> M2[Výplaty brokerům]

    A --> Pool[Pool program]
    A --> Audit[Audit log]
    A --> Support[Podpora / incidenty]
```
