# Tipari.cz – Kanonický souhrn dat (verze 1.3 – data)

**Stav dokumentu:** **DATA UZAMČENA (verze 1.3)**. Tento soubor je kanonický zdroj pravdy pro implementaci dat a business logiky.


**Verze:** 1.3 (data – uzamčeno)

**Datum uzamčení:** 2026-01-20

**Navazuje na:** interní pracovní revizi 6.0 (živý dokument)

## Dokumenty v balíčku (verze 1.3)

- **Souhrn dat (kanonický, uzamčeno)**: `Tipari_Souhrn_Dat_DATA_v1.3.md` (tento soubor)
- **Předchozí uzamčená verze (archiv)**: `Tipari_Souhrn_Dat_DATA_v1.2.md`
- **Starší archivní verze**: `Tipari_Souhrn_Dat_DATA_v1.0.md`
- **UX specifikace (pracovní)**: `Tipari_UX_Spec_v1.0.md`
- **UI specifikace (pracovní)**: `Tipari_UI_Spec_v1.0.md`
- **Archiv (původní spojený soubor, včetně historických poznámek)**: `Tipari_Souhrn_Dat_a_Pravidel_DATA_v1.0_LOCKED.md`

> Pravidlo konzistence: **data a business logika se mění pouze v tomto souboru**. UX/UI dokumenty se na něj pouze odkazují a nesmí zavádět nové číselníky nebo pravidla bez synchronizace s daty.
...
- **Primární zdroj pravdy:** tento soubor (Kanonický souhrn dat).
- Níže jsou podklady, které jsme použili jako vstupy pro sjednocení a finální rozhodnutí. Jsou **referenční/archivní**.

Seznam vstupních podkladů (archiv):
- Zadání projektu do systému.docx
- Registrace obchodník developer.docx
- Vstupní data o investorovi.docx
- Formy financování vs zajištění.docx
- Formy investice.docx
- Formy zajištění.docx
- Znalecký posudek.docx
- Využití prostředků.docx
- Rozdělení provize.docx
- Brand a produktová identita.docx
- Nastavení admin.docx
- Tabulka práva a viditelnosti.docx
- Kanonický domain dictionary.docx
...
#### 0.4 Změnový log (revize 6.0 → 1.3 – data)

- Dokument je **uzamčený jako verze 1.3 – data** (tj. kanonická data a pravidla jsou připravená pro implementaci).
...
- Upřesnění fyzického podpisu investora: obchodník může zvolit, zda investor podepíše **Souhlas + NDA** elektronicky (eSign) nebo fyzicky (scan). Při fyzické variantě se nahrávají samostatně: Souhlas, NDA a následně rezervační smlouva.
...
- Podpisové dokumenty investora: **Souhlas se sdílením údajů** + **NDA (investor ↔ platforma)** (elektronický podpis / fyzický podpis + nahraný scan)
...
- Rezervace: koncept → odesláno investorovi (čeká na Souhlas + NDA) → Souhlas + NDA potvrzeny (odmaskováno) → čeká na podpis rezervační smlouvy investorem → podepsáno investorem (v pořadí) → (v kapacitě / mimo kapacitu) → čeká na podpis developera → aktivní → jednání → profinancováno → uzavřeno
...
- Schválení a publikace projektu/tiketu (včetně nastavení: publikační okno, kapacita rezervací, časové lhůty podpisů – investor Souhlas+NDA, investor rezervační smlouva, developer podpis/rozhodnutí – a lhůty jednání)
...
4) obchodník odešle investorovi odkaz na elektronický podpis (nebo vygeneruje PDF balíček pro fyzický podpis). Od tohoto okamžiku běží SLA pro investora (**výchozí 48 hodin**, nastavitelné administrátorem na úrovni tiketu).
5) investor nejprve podepíše (standardně elektronicky v eSign; alternativně fyzicky s nahráním scanu obchodníkem):
...
7) pokud je rezervace v rámci kapacity tiketu, systém zpřístupní developerovi rezervační smlouvu podepsanou investorem a spustí SLA pro developera (**výchozí 48 hodin**, nastavitelné administrátorem na úrovni tiketu). Developer rezervaci:
   - **potvrdí** (podepíše rezervační smlouvu elektronicky, nebo fyzicky + nahraje scan), nebo
...
- odemknutí citlivých údajů směrem k developerovi je vázáno na podpis **Souhlasu se sdílením údajů** (investor → platforma + developer),
...
1) Obchodník vybere tiket a investora a vytvoří rezervaci (stav „Rozpracovaná“).
   - Do chvíle, než obchodník rezervaci **potvrdí a odešle**, vidí u projektu/tiketu **maskované**: **název projektu**, **název developera**, **obrázky projektu** a většinu dokumentů.
   - Vidí však základní parametry tiketu (např. **typ projektu**, **lokalitu**, **částku**, **formu financování**) a dokumenty, které administrátor ručně označí jako **„předrezervační“** (teaser).

2) Systém ověří základní podmínky:
   - tiket je ve stavu umožňujícím nové rezervace (typicky **Zveřejněný**),
   - obchodník má právo vytvářet rezervace (aktivní účet, limity, sloty),
   - investor existuje v interní evidenci obchodníka (platforma investora neověřuje).

   Poznámka: Kapacita tiketu (např. 3) se v tomto kroku **neblokuje** – pořadí a „právo na rezervaci“ vzniká až podpisem **rezervační smlouvy investorem** (viz krok 6).

3) Obchodník rezervaci **potvrdí a odešle** investorovi. Od této chvíle běží SLA pro investora (výchozí **48 hodin**; administrátor může měnit i prodlužovat – per tiket i per rezervaci).

   **Varianta A – Elektronický podpis (výchozí):**
   - Investor dostane e-mail s odkazem na podpisový proces.
   - V e-mailu jsou v příloze připravené dokumenty:
     - **Souhlas se sdílením údajů** (platforma + developer),
     - **NDA** (investor ↔ platforma),
     - (volitelně) informační PDF o tiketu.

   **Varianta B – Fyzický podpis investora (scan):**
   - Obchodník zvolí, **jak investor podepíše Souhlas + NDA**:
     - elektronicky (eSign) **nebo**
     - fyzicky (scan).
   - **Rezervační smlouva** se v této variantě podepisuje fyzicky mimo platformu a obchodník nahraje scan do rezervace.
   - Pokud obchodník zvolí fyzický podpis i pro Souhlas + NDA, systém umožní stáhnout tiskový PDF balíček (**Souhlas + NDA + Rezervační smlouva**) a obchodník následně nahraje **3 samostatné dokumenty**: Souhlas (scan), NDA (scan), Rezervační smlouva (scan).

4) Investor nejprve podepisuje **Souhlas se sdílením údajů** a **NDA** (v e‑sign okně jako první krok; nebo fyzicky přes scany).

   Jakmile je Souhlas + NDA potvrzen:
   - Platforma zapíše auditní stopu, že **investor i obchodník byli developerovi „představení“ platformou**.
   - Obchodníkovi se **odmaskují** detaily projektu a developera (název, obrázky, kompletní dokumenty projektu podle nastavení viditelnosti).
   - Developerovi se v dashboardu zobrazí „probíhající rezervace“ včetně **jména investora a obchodníka**, ve stavu „Souhlas + NDA potvrzeny – čeká na podpis rezervační smlouvy investorem“.

   Pokud investor Souhlas + NDA nepotvrdí do SLA:
   - rezervace zanikne jako „neúspěšná – investor nepotvrdil Souhlas/NDA“,
   - developer o rezervaci vůbec neuvidí (a nedojde k odkrytí identit ani dat).

5) Investor následně podepisuje **rezervační smlouvu** (druhý krok e‑sign; nebo fyzicky přes scan).

   Pokud investor rezervační smlouvu nepodepíše do SLA:
   - rezervace zanikne jako „neúspěšná – investor nepodepsal rezervační smlouvu“,
   - auditní záznam o podpisu Souhlasu + NDA (pokud proběhl) zůstává zachován.

6) Okamžikem podpisu rezervační smlouvy investorem vzniká **pořadí** na tiketu:
   - rozhoduje **datum a čas podpisu rezervační smlouvy investorem** (u fyzického podpisu datum a čas **nahrání scanu**),
   - systém seřadí všechny investorem podepsané rezervační smlouvy a určí, které jsou **v kapacitě** (výchozí kapacita 3; nastavitelná administrátorem; může být navržena developerem nebo obchodníkem při zakládání tiketu).

   Rezervace mimo kapacitu jsou ve stavu „ve frontě“ a systém je automaticky posune, jakmile se uvolní místo (zamítnutí, expirace, zánik).

7) Developer může potvrdit/zamítnout pouze rezervace, které jsou **v kapacitě**.

   Jakmile je rezervace v kapacitě:
   - systém developerovi zpřístupní investorem podepsanou rezervační smlouvu,
   - spustí SLA pro developera (výchozí **48 hodin**; administrátor může měnit i prodlužovat).

   **Možnosti developera:**
   - **Potvrdit** = podepsat rezervační smlouvu (elektronicky nebo fyzicky + scan),
   - **Zamítnout** = povinné odůvodnění.

   Pokud developer v termínu nepodepíše / zamítne:
   - rezervace se ukončí jako neúspěšná,
   - systém posune další rezervaci z fronty do kapacity.

8) Po podpisu obou stran se rezervace stává **Aktivní**:
   - od aktivace běží jednání / okno na profinancování (default **30 dní**, nastavitelné/prodloužitelné administrátorem),
   - identifikace stran je již odemčená z kroku 4; aktivace potvrzuje právní závaznost rezervační smlouvy.

9) Ve chvíli, kdy jakákoliv rezervace na tiketu dosáhne stavu „Financování potvrzeno“:
   - tiket se uzavře jako profinancovaný,
   - všechny ostatní rezervace (včetně čekajících) automaticky zanikají,
   - systém informuje dotčené obchodníky/investory, že financování bylo realizováno jiným investorem.
...
- po **odeslání** investorovi běží SLA pro investora (výchozí 48h) na: **Souhlas + NDA** a následně podpis **rezervační smlouvy**
...
- obsahuje metodu podpisu (elektronicky / fyzicky + scan), termíny, dokumenty, auditní stopu a vazbu na provizi
...
- Vypršela lhůta pro Souhlas + NDA investora
- Vypršela lhůta pro podpis rezervační smlouvy investorem
...
- Obchodník vidí u projektu/tiketu pouze **maskované** informace: název projektu, název developera, obrázky projektu a většinu dokumentů.
...
- **Souhlas se sdílením údajů** (platforma + developer) a
- **NDA** (investor ↔ platforma),
...
| Atribut | Typ | Povinné | Vlastník | Kdo vidí | Poznámka |
|---|---|---|---|---|---|
...
| Obrázky projektu (galerie) | list(file) | volitelně | developer | Developer, admin; obchodník až po podpisu Souhlasu + NDA investorem. | Před odmaskováním se zobrazuje placeholder. |
...
Poznámka: Maskování/odmaskování se řídí pravidly v kapitolách **4.5** a **10.2**.
...
| Čas na podpis investora | duration | ano | administrátor | administrátor | Výchozí 48h (Souhlas+NDA + rezervační smlouva investorem) |
...
| Datum a čas odeslání investorovi | datetime | ano | systém | admin, obchodník | Od tohoto okamžiku běží SLA pro investora. |
| Metoda podpisu investora | enum (elektronicky / fyzicky+scan) | ano | obchodník | admin, obchodník | Metoda platí pro Souhlas, NDA i rezervační smlouvu investorem. |
| Termín pro podpis investora | datetime | ano | systém | admin, obchodník | Deadline pro dokončení investorského podpisového procesu (Souhlas+NDA+rezervační smlouva). Default 48h; nastavitelné. |
| Dokument – Souhlas se sdílením údajů (k podpisu) | file/id | ano | systém | admin; investor (přes podpisový odkaz) | Souhlas je určen pro platformu + developera; obsah řeší právní sdílení osobních/firmních údajů. |
...
| Datum/čas podpisu rezervační smlouvy investorem | datetime | volitelné | systém | admin, obchodník; developer** | Používá se pro pořadí ve frontě. U fyzického podpisu = čas nahrání scanu. |
...
| Metoda podpisu developera | enum (elektronicky / fyzicky+scan) | ano | developer | admin, developer** | |
...
#### 19.1 Typy dokumentů (kanonické)

**Smlouvy / dokumenty mezi účastníky platformy:**

- Smlouva **developer ↔ platforma** (provizní podmínky, splatnosti, kontakty, ...).
- Smlouva **obchodník ↔ platforma** (fakturace, výplata provizí, pravidla práce s investorem, ...).

**Dokumenty v rámci rezervace (investor):**

- **Souhlas se sdílením údajů** (investor → platforma + developer).
  - slouží jako právní základ pro zpřístupnění osobních / identifikačních údajů investora developerovi a zároveň pro odmaskování detailů projektu obchodníkovi,
  - podepisuje investor **před** tím, než se dostane k rezervační smlouvě a plným detailům projektu.
- **NDA** (investor ↔ platforma).
  - podepisuje investor spolu se Souhlasem,
  - dokument je určen pro platformu (admin) – developer k němu nemá přístup.
- **Rezervační smlouva** (investor ↔ developer).
  - generovaná systémem,
  - podepisuje ji investor a následně developer,
  - aktivní rezervace vzniká až po podpisu obou stran.

**Projektové podklady (dokumenty projektu):**

- projekt může obsahovat soubory (PDF apod.) s atributem **viditelnosti**:
  - **Teaser / předrezervační** (zobrazí se obchodníkovi ještě před odesláním rezervace),
  - **Po Souhlasu + NDA** (zpřístupní se obchodníkovi až po podpisu Souhlasu + NDA investorem),
  - **Jen developer**,
  - **Jen administrátor**.

#### 19.2 Elektronický podpis (kanonický tok pro rezervaci)

- Obchodník odešle rezervaci investorovi → začne běžet SLA investora (výchozí 48h).
- Investor dostane e-mail s odkazem na podpisový proces.
  - V e-mailu jsou v příloze (nebo jako součást podpisového procesu) minimálně dokumenty: **Souhlas se sdílením údajů** a **NDA**.
- Podpisový proces je **sekvenční**:
  1) Investor podepíše **Souhlas se sdílením údajů** a **NDA**.
     - v tento okamžik:
       - obchodníkovi se odmaskují detaily projektu/developera,
       - developerovi se zobrazí probíhající rezervace a odemkne se mu identita investora i obchodníka,
       - platforma uloží auditní stopu (čas, metoda, vazba na dokumenty).
  2) Investor podepíše **rezervační smlouvu** (s plnými údaji o projektu a developerovi).
     - vzniká pořadí rezervace dle času podpisu investora,
     - investorem podepsaná smlouva se developerovi zpřístupní k podpisu až ve chvíli, kdy je rezervace **v kapacitě** tiketu.
- Developer má po zpřístupnění investorem podepsané smlouvy k podpisu SLA (výchozí 48h):
  - **potvrdit** = podepsat rezervační smlouvu,
  - **zamítnout** = povinné odůvodnění.

Audit log musí obsahovat minimálně: kdo, kdy, jakým způsobem podepsal, jaký dokument a k jaké rezervaci/tiketu.

#### 19.3 Fyzický podpis investora (PDF + scan)

Fyzický podpis je alternativní režim pro situace, kdy investor nepodepisuje jeden nebo více dokumentů elektronicky (eSign). V systému se pro investora volí způsob podpisu u každého dokumentu zvlášť:

- **Souhlas se sdílením údajů**: elektronicky (eSign) **nebo** fyzicky (scan)
- **NDA (investor ↔ platforma)**: elektronicky (eSign) **nebo** fyzicky (scan)
- **Rezervační smlouva**: elektronicky (eSign) **nebo** fyzicky (scan)

> Poznámka: Tato kapitola popisuje fyzickou variantu (scan). Elektronickou variantu řeší kap. 19.2.

##### 19.3.1 Varianta A – Souhlas + NDA elektronicky, rezervační smlouva fyzicky

1) Obchodník odešle rezervaci investorovi (SLA investora běží dle tiketu).
2) Investor podepíše **Souhlas se sdílením údajů** + **NDA** elektronicky v eSign.
3) Jakmile jsou Souhlas + NDA potvrzeny (odemknutí a auditní stopa):
   - obchodníkovi se odmaskují detaily projektu a developera,
   - developerovi se zobrazí probíhající rezervace (včetně identity investora a obchodníka).
4) Obchodník si stáhne systémem vygenerované PDF **rezervační smlouvy**, nechá investora podepsat fyzicky mimo platformu a nahraje scan.
5) **Čas nahrání scanu rezervační smlouvy** se používá jako **čas podpisu investora** pro:
   - **pořadí rezervace** (fronta / kapacita),
   - navazující stavové přechody (čeká na rozhodnutí developera).

##### 19.3.2 Varianta B – Souhlas + NDA fyzicky, rezervační smlouva fyzicky

1) Obchodník si nechá systémem vygenerovat tiskový balíček (minimálně):
   - Souhlas se sdílením údajů,
   - NDA,
   - Rezervační smlouva.
2) Investor podepíše dokumenty mimo platformu.
3) Obchodník nahraje scany do rezervace **v samostatných souborech**:
   - Souhlas (scan) – povinné,
   - NDA (scan) – povinné,
   - Rezervační smlouva (scan) – povinné (nahrává se až po Souhlasu + NDA).
4) **Časy nahrání scanů** se v systému používají jako časy podpisu investora:
   - Souhlas + NDA: pro účely **auditní stopy** a okamžiku **odemknutí** informací,
   - Rezervační smlouva: pro účely **pořadí rezervace** (fronta / kapacita).

##### 19.3.3 Společná pravidla pro fyzické podpisy (scan)

- Platforma obsah scanu **neověřuje**; scan slouží jako **evidence a audit**.
- Minimální požadavek na systém:
  - ke každému nahranému dokumentu musí být uloženo: typ dokumentu (Souhlas / NDA / Rezervační smlouva), kdo nahrál, datum a čas nahrání, vazba na rezervaci a tiket.
  - metoda podpisu = **fyzicky (scan)**.
- SLA investora (výchozí 48 hodin, nastavitelné administrátorem na úrovni tiketu) platí i pro fyzickou variantu:
  - pokud investor (resp. obchodník) nedodá požadované podepsané dokumenty včas, rezervace zaniká dle stavového automatu.


#### 19.4 Fyzický podpis developera (PDF + scan)

- Developer si stáhne investorem podepsanou rezervační smlouvu (je dostupná pouze pro rezervace v kapacitě).
...
| Odesláno investorovi (čeká na Souhlas + NDA) | Rezervace byla odeslána investorovi k podpisu, čeká se na potvrzení Souhlasu se sdílením údajů a NDA | obchodník odešle e-sign link / připraví fyzický balíček | Běží SLA investora (výchozí 48 hodin). Developer stále nevidí rezervaci. |
| Souhlas + NDA potvrzeny (čeká na podpis rezervační smlouvy investorem) | Investor podepsal Souhlas + NDA a může pokračovat k rezervační smlouvě | podpis investora Souhlas+NDA (e-sign nebo scan) | Vzniká auditní stopa introdukce. Obchodníkovi se odmaskují detaily projektu/developera. Developer uvidí probíhající rezervaci včetně jména investora a obchodníka (zatím bez akce). |
