...
**Stav dokumentu:** sjednocení, úklid a finalizace dat + zadání pro UX/UI (bez technických kódů a bez návrhu databáze)
...
Kanonický princip:
- **Provize obchodníka (základ)** se vždy počítá z **reálně profinancované částky** (částka, která přišla na bankovní účet developera) – zdrojem pravdy je potvrzení developera.
- Základ provize (odměna) je vždy počítaný jako podíl z profinancované částky **bez ohledu na to, zda je obchodník plátcem daně z přidané hodnoty**.
- **Daň z přidané hodnoty (DPH)** se k provizi připočítává **jen pokud je obchodník plátcem DPH**:
  - pokud je plátce, systém připočítá DPH a počítá "celkem k úhradě" = základ provize + DPH,
  - pokud není plátce, DPH = 0 a "celkem k úhradě" = základ provize.
- **Sazba DPH:** typicky 21 % (administrátor může upravit; u dokladu musí odpovídat faktuře obchodníka).
...
1) Developer potvrdí profinancování:
   - zadá **datum přijetí** financí na účet,
   - zadá **finální profinancovanou částku** (může se lišit od původního plánu),
   - volitelně nahraje podklady (doporučeno; viz kapitola 20).
...
3) Po přijetí úhrady provize platformě (ručně potvrzeno administrátorem) systém:
   - zpřístupní obchodníkům podklady k fakturaci (základ provize v Kč, informace o plátcovství DPH, výchozí sazba DPH, vypočtená DPH a celkem k úhradě, identifikace tiketu/projektu a reference platby).
4) Obchodník vystaví fakturu platformě:
   - pokud je plátcem daně z přidané hodnoty, připočte ji podle legislativy,
   - pokud není plátcem, fakturuje pouze základ.
5) Platforma uhradí fakturu obchodníka (výchozí lhůta 3 dny po přijetí faktury; upravitelné administrátorem).

Poznámka:
- Platforma poskytuje obchodníkům **fakturační podklady včetně částky DPH** (pokud je obchodník plátce) a počítá "celkem k úhradě". Obchodník však odpovídá za správnost daňového režimu a údajů na faktuře; pokud se faktura liší od podkladů, administrátor rozhodne ručně a provede auditní záznam.
...
- Obrat se počítá v **korunách českých**.
  - Pokud je tiket veden v jiné měně (například euro), musí být k profinancování uložen převodní kurz použitý pro přepočet na koruny české (v první verzi ručně; později lze automatizovat).
...
| Daňové identifikační číslo (DIČ) | text | podmíněně | obchodník | administrátor, obchodník | povinné pokud je obchodník plátce DPH |
| Plátce DPH | výběr: ano/ne | ano | obchodník | administrátor, obchodník | určuje, zda se k provizi připočítává DPH |
...
- Systém počítá provize a podíly **v korunách českých jako základ** (bez daně z přidané hodnoty).
- Pokud je obchodník **plátce DPH**, platforma k jeho provizi připočítá DPH (podle nastavené sazby) a eviduje i "celkem k úhradě".
- Pokud obchodník **není plátce DPH**, DPH = 0 a hradí se pouze základ provize.
- Platforma může provést základní kontrolu (základ × sazba = DPH), ale v případě odchylky (např. specifický režim) rozhoduje administrátor ručně a vše se zapíše do auditu.
...
- DPH u provize obchodníka: platforma k základu provize připočítává DPH pouze pokud je obchodník plátce DPH (viz kap. 10.6 a 12.9.4).


#### 20.2 Otevřené otázky

1) Doklady a audit u potvrzení financování a úhrad
- Jaké (minimální) podklady má developer přikládat při potvrzení financování (například potvrzení o převodu / bankovní výpis / jiné)?
- Návrh pro MVP: podklady jsou **volitelné**, ale systém umožní přiložit soubor + uloží důvod, pokud podklady přiloženy nejsou.

2) Měnové přepočty (pokud povolíme euro)
- Zda má systém v první verzi používat ruční kurz zadaný administrátorem, nebo automatický kurz z veřejného zdroje.
...
| Preference | Preferovaná měna | Ne | Obchodník | Výběr (enum) | CZK / EUR | matching |
...
| Parametry | Investiční částka (cílová) | Ano | Developer / Obchodník | Číslo + měna | >0 | Zobrazuje se na kartě tiketu |
| Parametry | Měna | Ano | Developer / Obchodník | Výběr z nabídky | CZK / EUR (default CZK) |  |
...
| Daň z přidané hodnoty (DPH) – sazba | Podmíněně | Obchodník | Výběr | Pouze pokud je obchodník plátce DPH | Sazba typicky 21 %, ale necháváme jako datové pole |
