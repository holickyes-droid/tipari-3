# Tipari.cz – Souhrn dat a pravidel (verze 1.1 – DATA)

**Stav dokumentu:** **DATA UZAMČENA (verze 1.1)**. Tento soubor je **kanonický zdroj pravdy** pro implementaci dat, entit, stavů a business logiky.

**Verze:** 1.1 (DATA)

**Datum uzamčení:** 2026-01-20

## Související soubory (doporučené rozdělení dokumentace)

- **UX specifikace (pracovní):** `Tipari_UX_SPEC_v0.1.md`
- **UI specifikace (pracovní):** `Tipari_UI_SPEC_v0.1.md`
- **Decision Log (historie nesrovnalostí a rozhodnutí):** `Tipari_Decision_Log_v1.0.md`

> Poznámka: Tento soubor je záměrně **očištěn** od sekcí typu „nesrovnalosti v podkladech“ a „naše kanonické rozhodnutí“. Všechny takové poznámky jsou přesunuty do **Decision Logu**.
...
- **Primární zdroj pravdy:** tento soubor (Souhrn dat a pravidel – DATA). Všechny další podklady níže jsou pouze referenční/archivní a nesmí přepisovat pravidla uvedená v kapitolách 8–21.
...
- **Kanonický domain dictionary.docx** (referenční slovník; obsahuje rozpory s novějšími podklady – viz Decision Log)
...
- Dokument je **uzamčený jako verze 1.1 – data** (tj. kanonická data a pravidla jsou připravená pro implementaci).
...
- Rezervace: rozpracovaná → odesláno investorovi → čeká na Souhlas + NDA investora → Souhlas + NDA podepsáno (odemknutí dat) → čeká na podpis rezervační smlouvy investorem → podepsáno investorem (pořadí; ve frontě / v kapacitě) → čeká na podpis/rozhodnutí developera → aktivní → jednání → profinancováno → uzavřeno
...
- **Obchodník (tipař/broker):** vidí nabídku projektů a tiketů, ale klíčové identifikátory jsou z důvodu ochrany stran **maskované**. Plný detail projektu a developera (včetně obrázků a plných dokumentů) se obchodníkovi **odmaskuje až ve chvíli, kdy investor podepíše Souhlas se sdílením údajů + NDA**.
- **Developer:** vidí své projekty a tikety. Identitu investora a obchodníka uvidí **už po podpisu Souhlasu + NDA investorem** (auditní důkaz „představení“ investora developerovi platformou). Rezervaci však schvaluje/podepisuje až po tom, co investor podepíše rezervační smlouvu a rezervace se dostane do kapacity tiketu.
- **Platforma (administrátor):** schvaluje a publikuje obsah, nastavuje parametry (SLA, kapacity, doby), eviduje auditní stopu a umožňuje ruční zásahy. Platforma investora **neověřuje** a neschvaluje, kdo si koho broker eviduje.

#### 1.3 Přehled platformy
Základní tok je:

1. Developer (nebo obchodník) založí projekt/tiket → administrátor schválí a publikuje.
2. Obchodník vybere tiket a investora ze své interní databáze → založí rezervaci a odešle ji investorovi k podpisu přes eSign.
3. Investor (mimo platformu, bez účtu) v eSign postupně podepíše:
   - Souhlas se sdílením údajů (platforma + developer),
   - NDA s platformou,
   - rezervační smlouvu (v tomto kroku již vidí plné údaje o projektu a developerovi).
4. Po podpisu Souhlasu + NDA:
   - obchodníkovi se odmaskují plné údaje o projektu a developerovi,
   - developer uvidí v dashboardu investora i obchodníka (auditní stopa představení).
5. Po podpisu rezervační smlouvy investorem systém určí pořadí a kapacitu (čas podpisu / čas nahrání scanu).
6. Developer rezervaci podepíše nebo zamítne s odůvodněním (eSign nebo fyzicky + scan).
7. Po podpisu obou stran se rezervace aktivuje a běží jednání (výchozí 30 dní, prodlužitelné administrátorem).
8. Po financování developer potvrdí přijetí peněz na účet (datum + částka) → vzniká nárok na provizi platformy a následné fakturace/výplaty dle pravidel.

Poznámka: Pokud investor podepíše Souhlas + NDA, ale nestihne podepsat rezervační smlouvu, rezervace po vypršení SLA zaniká, auditní stopa Souhlasu + NDA se ale zachová.
...
- Rezervace je **aktivní** až po podpisu rezervační smlouvy oběma stranami (investor + developer). **Odemknutí identit v UI** (projekt/developer pro obchodníka a investor/obchodník pro developera) probíhá už po podpisu **Souhlasu + NDA investorem** – to slouží jako auditní důkaz „představení“.
...
Rezervace tiketu je řízený proces, který kombinuje:
- soutěž o kapacitu tiketu (pořadí podle času podpisu investora na **rezervační smlouvě**),
- právní/auditní stopu „představení investora developerovi platformou“ (momentem podpisu **Souhlasu se sdílením údajů + NDA** investorem),
- práci s dokumenty (rezervační smlouva může být e‑podpis nebo fyzický podpis + scan).

**Důležité principy (kanonické):**
- Investor **nemá účet na platformě**; veškeré kroky investora probíhají **mimo platformu** přes službu elektronického podpisu (eSign) / externí podpisové okno.
- Dokud obchodník rezervaci **neodešle** investorovi a investor nepodepíše Souhlas + NDA, obchodník vidí u tiketu/projektu pouze **zamaskované** údaje a omezený náhled.
- Odemknutí detailů projektu/developera obchodníkovi a zobrazení investora+obchodníka developerovi nastává **už po podpisu Souhlasu + NDA investorem** (auditní důkaz introdukce).
- Rezervace se stává **aktivní** až po podpisu **rezervační smlouvy oběma stranami** (investor + developer).

##### 4.5.1 Viditelnost před odesláním rezervace (maskování)

Obchodník před odesláním rezervace vidí u tiketu/projektu pouze:
- **zamaskované**: název projektu, název developera, obrázky k projektu (omezeně), plné dokumenty,
- **povolené náhledy**: dokumenty a materiály, které administrátor ručně označí jako „preview“ (např. anonymizovaný teaser).

Ostatní dokumenty, plné obrázky a plné údaje jsou dostupné až po podpisu Souhlasu + NDA investorem.

##### 4.5.2 Krok 1 – založení rezervace obchodníkem

1. Obchodník vybere tiket a vybere investora ze své interní databáze (CRM).
2. Systém vytvoří rezervaci ve stavu **Rozpracovaná**.
3. Obchodník zkontroluje základní parametry (maskovaně) a potvrdí **odeslání** rezervace investorovi.

##### 4.5.3 Krok 2 – odeslání do eSign (investor)

4. Po odeslání systém vytvoří podpisový proces v eSign a odešle investorovi e-mail se signing linkem.
5. Podpisový tok investora je **sekvenční**:
   - **Souhlas se sdílením údajů** (platforma + developer)
   - **NDA s platformou** (jen platforma)
   - **Rezervační smlouva** (investor vidí kompletní údaje o projektu a developerovi až po podpisu Souhlasu + NDA)
6. Od okamžiku odeslání běží SLA pro investora: **48 hodin** (admin může prodloužit). Investor musí do této lhůty dokončit požadované podpisy.

##### 4.5.4 Krok 3 – podpis Souhlasu + NDA investorem (auditní odemknutí)

7. Jakmile eSign potvrdí podpis Souhlasu a NDA:
   - systém uloží čas podpisu Souhlasu a NDA,
   - obchodníkovi se **odmaskují** údaje o projektu a developerovi (včetně plných obrázků a dokumentů),
   - developerovi se v dashboardu zobrazí **probíhající rezervace** včetně jména investora a obchodníka,
   - vzniká auditní stopa, že investor (a obchodník) byl developerovi představen platformou.

> Pokud investor podepíše Souhlas + NDA, ale **nepodepíše rezervační smlouvu**, rezervace může expirovat po SLA. Auditní stopa Souhlasu + NDA zůstává uložená.

##### 4.5.5 Krok 4 – podpis rezervační smlouvy investorem (pořadí a kapacita)

8. Jakmile investor podepíše rezervační smlouvu:
   - systém uloží rezervační smlouvu podepsanou investorem (PDF) a čas podpisu,
   - **pořadí rezervace** se určuje podle času podpisu investora na rezervační smlouvě (u fyzického podpisu dle času nahrání scanu),
   - systém přepočítá kapacitu a určí, které rezervace jsou **v kapacitě**.

##### 4.5.6 Krok 5 – potvrzení nebo zamítnutí developerem

9. Developer může rezervaci:
   - **potvrdit podpisem** rezervační smlouvy (eSign nebo fyzicky + scan),
   - nebo **zamítnout** s odůvodněním.
10. SLA pro developera na podpis/rozhodnutí je výchozí **48 hodin** od okamžiku, kdy rezervace vstoupí do kapacity (admin může prodloužit).
11. Po podpisu rezervační smlouvy oběma stranami se rezervace stává **Aktivní** a začne běžet jednací období (výchozí 30 dní; admin může měnit/prodlužovat).

##### 4.5.7 Důsledky financování

12. Jakmile některá rezervace na tiketu dojde do financování, všechny ostatní rezervace na tiketu zanikají a obchodníci/investoři jsou informováni, že tiket byl profinancován jiným investorem.
...
- Maskování identity projektu/developera a investora je klíčový právně-obchodní mechanismus; odemknutí identit pro obchodníka a zobrazení investora + obchodníka developerovi nastává po podpisu Souhlasu se sdílením údajů + NDA investorem (auditní důkaz introdukce). Aktivace rezervace je až po podpisu rezervační smlouvy oběma stranami.
...
- Rozpracovaná (obchodník připravuje rezervaci; ještě neodesláno investorovi)
- Odesláno investorovi (čeká na Souhlas + NDA investora; běží SLA)
- Souhlas + NDA podepsáno (odemknutí dat; čeká na podpis rezervační smlouvy investorem)
- Podepsáno investorem – ve frontě (mimo kapacitu)
- Podepsáno investorem – v kapacitě (čeká na podpis/rozhodnutí developera)
- Aktivní (rezervační smlouva podepsána oběma stranami; běží jednání)
- Financováno (developer potvrdil přijetí peněz na účet)
- Ukončeno neúspěšně
- Spor
...
#### 10.2 Odemknutí identity a zpřístupnění detailů během rezervace

Platforma pracuje s **maskováním** a **postupným odemknutím** informací, aby (a) chránila obchodní model proti obcházení a (b) měla auditní důkaz, kdy došlo k představení investora developerovi.

Kanonické momenty zpřístupnění:

1) **Před odesláním rezervace** (rezervace je Rozpracovaná)
- Obchodník vidí u projektu/tiketu pouze **zamaskované** údaje (název projektu, název developera, obrázky/dokumenty omezeně).
- Obchodník může vidět jen dokumenty označené administrátorem jako „preview“.
- Developer o této rezervaci neví.

2) **Po podpisu Souhlasu se sdílením údajů + NDA investorem**
- Investor podepisuje Souhlas + NDA mimo platformu (eSign).
- Systém tímto okamžikem:
  - **odmaskuje obchodníkovi** plné údaje o projektu a developerovi,
  - **zobrazí developerovi** probíhající rezervaci včetně jména investora a obchodníka,
  - založí auditní stopu introdukce (čas, dokumenty, identifikátory podpisu).

3) **Po podpisu rezervační smlouvy investorem**
- Rezervace se zařadí do pořadí podle času podpisu investora na rezervační smlouvě (u fyzického podpisu dle času nahrání scanu).
- Systém přepočítá kapacitu a určí, které rezervace jsou „v kapacitě“ (a tedy čekají na krok developera).

4) **Po podpisu rezervační smlouvy oběma stranami**
- Rezervace se stává **Aktivní**.
- Začne běžet jednací období (výchozí 30 dní od aktivace; admin může měnit/prodlužovat).

Pozn.: Odemknutí identit není vázané na aktivaci rezervace – probíhá dříve (po podpisu Souhlasu + NDA investorem). Aktivace je obchodní stav, který spouští jednání a další lhůty.
...
**Maskování identity:**
- Identita investora a identita projektu (název projektu a developer) se v uživatelském rozhraní odemykají až po podpisu Souhlasu + NDA investorem.
...
| Pole | Typ / formát | Povinnost | Vyplňuje | Kdo vidí | Validace / poznámka |
|---|---|---|---|---|---|
| Název projektu | text | ano | developer | administrátor, developer; obchodník až po podpisu Souhlasu + NDA investorem | maskováno do podpisu Souhlasu + NDA investorem |
...
| Přesná adresa | text | volitelné | developer | administrátor, developer; obchodník až po podpisu Souhlasu + NDA investorem | citlivé |
...
| Dokumenty projektu | soubory | volitelné | developer | administrátor, developer; obchodník: preview před odesláním rezervace (dle nastavení) + full po podpisu Souhlasu + NDA investorem | přístup řízený |
...
| Kapacita rezervací na tiketu (celkem) | integer | ano | administrátor | administrátor, developer | Výchozí 3; **do kapacity se počítají rezervace od podpisu investora na rezervační smlouvě** (čeká na podpis developera / aktivní / jednání). Rezervace před podpisem investora kapacitu neblokují. |
...
Rezervace je vazba mezi **tiketem** a **investorem obchodníka**. Slouží k řízení kapacity tiketu, podpisů a následného procesu jednání/financování.

| Pole | Typ | Povinné | Vidí obchodník | Vidí developer | Poznámka | 
|---|---|---:|---:|---:|---|
| ID rezervace | string | Ano | Ano | Ano | Interní identifikátor. |
| Tiket | TicketRef | Ano | Ano | Ano | Vazba na tiket. |
| Investor | InvestorRef | Ano | Ano | Ano (po Souhlasu+NDA) | Developer vidí investora až po podpisu Souhlasu + NDA investorem. |
| Obchodník | BrokerRef | Ano | Ano | Ano (po Souhlasu+NDA) | Developer vidí obchodníka až po podpisu Souhlasu + NDA investorem. |
| Stav | ReservationStatus | Ano | Ano | Ano | Stavový automat v kap. 21.2. |
| Vytvořeno | datetime | Ano | Ano | Ano | Čas vytvoření rozpracované rezervace. |
| Odesláno investorovi | datetime | Ne | Ano | Ne | Čas odeslání do eSign (start SLA pro investora). |
| Termín (SLA) pro investora | datetime | Ne | Ano | Ne | Výchozí 48 h od odeslání; investor musí v lhůtě podepsat Souhlas+NDA a rezervační smlouvu. |
| Souhlas se sdílením údajů – podepsáno | datetime | Ne | Ano | Ano | Po podpisu se odemknou data (Moment B). |
| NDA (jen platforma) – podepsáno | datetime | Ne | Ano | Ano | Součást auditní stopy; developer není smluvní stranou NDA. |
| Rezervační smlouva – podepsáno investorem | datetime | Ne | Ano | Ano | Podle tohoto času se určuje pořadí (u scanu je to čas nahrání). |
| Rezervační smlouva – podepsáno developerem | datetime | Ne | Ano | Ano | Po podpisu obou stran přechod do Aktivní. |
| Termín (SLA) pro developera | datetime | Ne | Ano | Ano | Výchozí 48 h od vstupu rezervace do kapacity (po podpisu investora). |
| Dokument: Souhlas se sdílením údajů | DocumentRef | Ne | Ano | Ne | Podepsaný dokument uložený pro audit. |
| Dokument: NDA | DocumentRef | Ne | Ano | Ne | Podepsaný dokument uložený pro audit. |
| Dokument: Rezervační smlouva (finální) | DocumentRef | Ne | Ano | Ano | Finální verze rezervační smlouvy (po podpisech). |
| eSign identifikátor (envelope) | string | Ne | Ano | Ne | Identifikátor transakce u eSign (pro audit / troubleshooting). |
| Důvod ukončení | string / enum | Ne | Ano | Ano | Vyplněno při ukončení (SLA, zamítnutí, profinancováno jiným investorem, expirace tiketu…). |
| Odůvodnění zamítnutí (developer) | text | Ne | Ano | Ano | Povinné, pokud developer zamítne. |
| Poznámka administrátora | text | Ne | Ano | Ano | Volitelné. |
...
- Varianta fakturace „obchodník → developer“ se v této verzi platformy **nepoužívá**.
...
- založení rezervace,
- odeslání eSign pozvánky investorovi (mimo platformu),
- podpis Souhlasu + NDA investorem (odemknutí identit; auditní stopa introdukce),
- podpis investora na rezervační smlouvě (včetně času podpisu – rozhoduje pořadí),
- podpis/rozhodnutí developera (podpis nebo zamítnutí s odůvodněním),
- aktivace rezervace (rezervační smlouva podepsána oběma stranami; start jednání),
...
- Matching musí respektovat maskování identity projektu před podpisem Souhlasu + NDA investorem (obchodník pracuje primárně s parametry pro rozhodnutí).
...
#### 19.2 Elektronický podpis (eSign) – tok investora

- Všechny kroky investora probíhají **mimo platformu** přes službu elektronického podpisu (eSign). Investor nemá účet na platformě.
- Obchodník po odeslání rezervace spouští eSign proces, který investor obdrží e-mailem (signing link).
- eSign tok investora je sekvenční:
  1) **Souhlas se sdílením údajů** (platforma + developer)
  2) **NDA s platformou** (jen platforma)
  3) **Rezervační smlouva** (plné údaje o projektu a developerovi)
- Po podpisu Souhlasu + NDA eSign informuje platformu (callback/webhook), která:
  - odemkne plné údaje projektu/developera obchodníkovi,
  - zobrazí investor + obchodník developerovi jako probíhající rezervaci (auditní stopa).
- Audit log musí obsahovat: kdo, kdy, jak podepsal + identifikátor transakce u eSign (např. envelope ID).

#### 19.3 Fyzický podpis rezervační smlouvy (PDF + scan)

- Fyzický podpis je **výjimečná alternativa** pro rezervační smlouvu (ne pro Souhlas+NDA).
- Investor nejprve podepíše Souhlas + NDA elektronicky v eSign (aby došlo k odemknutí dat a auditní stopě).
- Poté:
  - Obchodník stáhne systémem vygenerované PDF rezervační smlouvy, nechá investora podepsat fyzicky mimo platformu a nahraje scan.
  - Čas nahrání scanu se používá jako čas podpisu investora pro pořadí rezervace.
...
| Stav rezervace | Co to znamená | Kdo stav nastavuje | Klíčová pravidla |
|---|---|---|---|
| Rozpracovaná | Rezervace je založená obchodníkem, ale ještě nebyla odeslána investorovi | obchodník | Obchodník vidí maskované údaje; kapacitu tiketu neblokuje. |
| Odesláno investorovi | Rezervační proces odeslán do eSign; čeká se na Souhlas + NDA investora | systém | Běží SLA pro investora (48 h od odeslání, admin může prodloužit). |
| Souhlas + NDA podepsáno | Investor podepsal Souhlas + NDA; odemkly se údaje a vznikla auditní stopa | systém | Obchodník vidí plné údaje projektu/developera; developer vidí investora i obchodníka. Čeká se na podpis rezervační smlouvy investorem. |
| Podepsáno investorem – ve frontě | Investor podepsal rezervační smlouvu, ale rezervace je mimo kapacitu tiketu | systém | Pořadí podle času podpisu investora (u scanu podle času nahrání). |
| Podepsáno investorem – v kapacitě | Rezervace je v kapacitě; čeká na podpis/rozhodnutí developera | systém | Spouští SLA pro developera (48 h od vstupu do kapacity, admin může prodloužit). |
| Aktivní | Rezervační smlouva podepsána oběma stranami; běží jednací období | systém | Jednání výchozí 30 dní (admin může prodloužit); po financování se ostatní rezervace ukončí. |
| Financováno | Financování potvrzeno developerem (přijetí na účet) | developer | Uzavírá tiket; vzniká nárok platformy na provizi. |
| Ukončeno neúspěšně | Rezervace ukončena bez financování | systém / administrátor / developer | Důvod: SLA, zamítnutí, expirace tiketu, profinancováno jiným investorem, ruční ukončení adminem. |
| Spor | Rezervace je v režimu sporu | administrátor | Evidence incidentu; auditní stopa. |

##### 21.2.2 Přechody rezervace (zjednodušeně)

- Rozpracovaná → Odesláno investorovi (obchodník odešle rezervaci)
- Odesláno investorovi → Souhlas + NDA podepsáno (investor podepíše Souhlas + NDA v eSign)
- Souhlas + NDA podepsáno → Podepsáno investorem – ve frontě / v kapacitě (investor podepíše rezervační smlouvu)
- Podepsáno investorem – v kapacitě → Aktivní (developer podepíše rezervační smlouvu)
- Jakýkoli „čekací“ stav → Ukončeno neúspěšně (SLA vypršelo / admin ukončil / tiket expiroval)
- Aktivní → Financováno (developer potvrdí přijetí financí)
...
- **Expirace podpisu investora:** pokud investor nedokončí podpisový tok do termínu, rezervace přejde do „Ukončeno neúspěšně“. Důvod se rozlišuje:
  - **nepodepsal Souhlas+NDA** (investor nepodepsal Souhlas se sdílením údajů a/nebo NDA),
  - **nepodepsal rezervační smlouvu** (Souhlas+NDA podepsáno, ale rezervační smlouva ne).

  V obou případech se zachová auditní stopa toho, co již bylo podepsáno (zejména podpis Souhlasu+NDA je důkaz „představení investora developerovi platformou“).
...
- podpis investora na rezervační smlouvě (nový vstup do fronty),
...
- V této verzi **neexistuje přímý provizní vztah obchodník ↔ developer**. Provize se účtuje developerovi vůči platformě a obchodník fakturuje svůj podíl platformě.
...
- Identita investora se developerovi odemkne po podpisu Souhlasu se sdílením údajů + NDA investorem (auditní důkaz introdukce). Aktivace rezervace (podpis rezervační smlouvy oběma stranami) spouští jednací období.
...
- Investor není uživatelský účet; pracujeme s minimalizací osobních údajů a s odkrytím identit na základě podpisu Souhlasu se sdílením údajů + NDA investorem.
