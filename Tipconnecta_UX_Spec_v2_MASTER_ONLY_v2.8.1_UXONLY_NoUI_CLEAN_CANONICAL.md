...
**Verze:** v2.8.1 (UX ONLY)
**Datum poslední aktualizace:** 2026-01-25
**Jazyk aplikace:** CZ  
...
- Základní parametry (bez detailů, dle `masking_level`):
  - typ investice
  - výnos p.a. (volitelně)
  - splatnost
  - lokalita na úrovni „kraj“ (ne detailní adresa)
- Média:
  - cover/galerie jsou skryté nebo rozmazané (placeholder)
- Podklady:
  - zobrazit pouze bezpečné dokumenty (např. teaser PDF)
  - ostatní zamčené (disabled + tooltip)
- CTA (pokud relevantní):
  - „Vybrat investora“ (rezervace může spustit odemčení, dle obchodního pravidla)
  - nebo pouze „Požádat o odemčení“ (pokud existuje proces)

### UX pravidlo
- Vždy jasně komunikovat **proč** je obsah zamčený a **co** ho odemkne.
...
### Otevření modalu
- Ze seznamu tiketů: CTA **Vybrat investora**
- Z detailu tiketu: CTA **Vybrat investora**

### Obsah modalu
- Nadpis: **Zahájit rezervaci (1/2)**
- Podnadpis: „Vyberte investora pro tento tiket.“
- Souhrn tiketu (mini karta):
  - název projektu (nebo alias)
  - `ticket.public_id`
  - objem + výnos (minimálně)
- Sekce **Investoři**
  - Dropdown / select: „Vyberte investora ze seznamu…“
    - položka = jméno + e-mail (např. „Jan Novák (jan.novak@email.cz)“)
    - podporuje psaní a filtrování
  - CTA link: **+ Přidat nového investora**
    - otevře **[BRK-221] Investor — vytvořit**
  - Helper text: „Investor není uživatel platformy – přijde mu e-mail s podpisem.“
- Sekce **Poznámka pro investora (volitelné)**
  - textarea (např. default „Dobrý den, posílám k podpisu…“)

### Akce (footer)
- **Zrušit** (sekundární) – zavře modal bez změn
- **Pokračovat** (primární)
  - disabled dokud není vybraný investor
  - po kliku otevře **[BRK-230] Potvrzení rezervace (2/2)**

### Validace
- Investor je povinný.
- Poznámka volitelná (max délka dle pravidel, např. 500–1000 znaků).

### Edge cases
- Broker nemá žádné investory → dropdown empty state + CTA „Přidat nového investora“.
- Load error investorů → hláška + „Zkusit znovu“.

### Analytika (min)
- `brk_reservation_step1_opened` (ticket_id)
- `brk_reservation_investor_selected` (ticket_id, investor_id)
- `brk_reservation_note_changed` (ticket_id)
- `brk_reservation_step1_continue_clicked` (ticket_id, investor_id)
...
### Cíl
Potvrdit, že broker má souhlas klienta a spustit rezervaci (držení kapacity) + odeslání balíku k podpisu investorovi.
...
#### Časový limit
Warning box:
- „**48 hodin na celý balík dokumentů**“
- „Pokud nedojde k podpisu v tomto limitu, rezervace bude automaticky zrušena.“

#### Investor
- Jméno / název investora
- E-mail

#### Povinné potvrzení brokera
Checkbox (required):
- „Potvrzuji, že mám souhlas klienta k provedení závazné rezervace kapacity.“
- Helper: „Zároveň beru na vědomí 48h lhůtu pro podpis.“

### Akce (footer)
- **Zpět** (sekundární) → návrat na **[BRK-220]**
- **Potvrdit rezervaci** (primární)
  - disabled dokud není zaškrtnut checkbox
  - po potvrzení:
    - vytvoří rezervaci (status „Čeká na podpis“ / „Vyžádáno podepsání“),
    - odešle podpisový balík investorovi,
    - zobrazí toast „Rezervace vytvořena“,
    - přesměruje na detail rezervace **[BRK-241]** (nebo zavře modal a refresh detail).

### Chybové stavy
- ESign / odeslání selhalo → zobrazit hlášku a nabídnout:
  - „Zkusit znovu“
  - „Uložit jako koncept“ (pokud podporujeme) / nebo „Zavřít“

### Analytika (min)
- `brk_reservation_step2_opened` (ticket_id, investor_id)
- `brk_reservation_step2_confirm_clicked` (ticket_id, investor_id)
- `brk_reservation_created` (reservation_id)
...
### Cíl uživatele (Broker)
- Rychle vyhledat vhodný tiket podle **ID / názvu / lokality**.
- Porovnat klíčové parametry (**objem, výnos, splatnost, kapacita, expirace, zajištění, odměna**).
- Jedním klikem zahájit rezervaci (vybrat investora) a držet kapacitu.

### Cíl byznysu
- Zrychlit obsazování kapacity (vyšší konverze Tiket → Rezervace).
- Snížit chybovost (rezervace bez souhlasu, mimo kritéria, bez dokumentů).
- Transparentně komunikovat **maskování/odemčení** identity a podkladů.

### Primární CTA
- **Vybrat investora** → otevře modal **[BRK-220] Zahájit rezervaci (1/2)**.

### Sekundární akce
- Otevřít detail tiketu (klik na kartu / název) → **[BRK-210]**.
- Otevřít filtry, změnit řazení, vyhledat.
...
### 2) Informační banner (uzamčení)
Zobrazuje se, pokud alespoň jeden tiket má `masking_level != none`.

Text (doporučený):
- „Identita projektu se odemkne po podpisu Souhlasu se sdílením údajů. Do té doby jsou některé podklady zamčené.“

Volitelná CTA:
- „Zjistit více“ (nápověda / FAQ, neblokující).

### 3) Ovládací lišta (nad seznamem)
- **Vyhledávání**: placeholder „Hledat podle ID nebo názvu…“
  - hledá v `ticket.public_id` (např. TIK-2024-002),
  - `project.name` (pokud odmaskované),
  - `project.alias` / `project.masked_label` (pokud maskované).
- **Řazení** (dropdown):
  - default: **Nejbližší expirace**
  - další typicky: Nejvyšší výnos, Nejnižší LTV, Nejvyšší objem, Nejnovější (dle backendu).
- **Filtry** (button):
  - otevírá panel/drawer s filtry (UI může být „Filtry“ vpravo).
  - minimální sada filtrů:
    - Stav (Aktivní / Končí brzy / Poslední místa / Uzavřeno / Naplněno)
    - Lokalita (kraj/město)
    - Typ investice (Seniorní úvěr / Mezanin / Equity)
    - Zajištění (1. pořadí / 2. pořadí / Bez zajištění / …)
    - Rozsahy: objem, výnos, splatnost, LTV
    - Maskování: „Zobrazit i uzamčené“ (default ON)
  - filtry se aplikují okamžitě; u tlačítka zobrazit počet aktivních filtrů.
...
### A) Náhled (media)
- Pokud `masking_level = full_mask`:
  - zobrazit placeholder s ikonou zámku + label **ZAMČENO**
- Pokud `masking_level = partial_mask`:
  - může být zobrazen thumbnail, ale přidat badge **ČÁSTEČNĚ UZAMČENO**
- Pokud odmaskované:
  - zobrazit thumbnail = **hlavní obrázek projektu** (`project.media.cover`)
- Klik na náhled otevře **[BRK-210] detail tiketu**.

### B) Hlavní info + KPI
- ID štítek: `ticket.public_id` (např. TIK-2024-002)
- Badge stavu (dle pravidel níže):
  - `ČÁSTEČNĚ UZAMČENO`
  - `POSLEDNÍ MÍSTA`
  - `KONČÍ BRZY`
  - `NAPLNĚNO` / `UZAVŘENO`
- Název projektu:
  - odmaskované: `project.name`
  - maskované: `project.alias` / `project.masked_label`
- Lokalita: `project.location_city` (+ případně kraj)
- Expirace:
  - datum `ticket.expires_at`
  - relativně „(za X dní)“
- KPI blok (minimální):
  - **Objem** (`ticket.total_volume_czk`)
  - **Výnos p.a.** (`ticket.yield_pa_pct`)
  - **Splatnost** (`ticket.maturity_months`)
- Kapacita:
  - progress bar `ticket.capacity_filled_pct`
  - text `ticket.capacity_filled_count / ticket.capacity_total_count` (např. 2/3)

### C) Pravý sloupec (akce)
- Zajištění:
  - label „Zajištění“ + hodnota (např. „1. v pořadí“, „2. v pořadí“, „Equity“)
- Odměna brokera:
  - částka bez DPH `ticket.broker_fee_net_czk`
- Button:
  - aktivní tiket: **Vybrat investora**
  - uzavřeno/naplněno/expirace: disabled + tooltip „Tiket není možné rezervovat.“

---

## Pravidla badge / stavy
- **KONČÍ BRZY**: pokud `days_to_expire <= X` (typicky 7)
- **POSLEDNÍ MÍSTA**: pokud `remaining_capacity <= threshold` (typicky 1–2 pozice nebo <10 %)
- **NAPLNĚNO**: pokud `capacity_remaining = 0`
- **UZAVŘENO**: pokud `ticket.status in {closed, expired, cancelled}`

---

## Empty / error stavy
- Bez výsledků: „Žádné tikety neodpovídají filtru.“ + CTA „Zrušit filtry“
- Chyba načtení: „Nepodařilo se načíst tikety.“ + CTA „Zkusit znovu“

---

## Analytika (minimální eventy)
- `brk_tickets_list_opened`
- `brk_tickets_search_used`
- `brk_tickets_filters_changed`
- `brk_ticket_card_opened` (ticket_id)
- `brk_ticket_select_investor_clicked` (ticket_id)

## [BRK-210] Tiket — Detail (Broker)

### Cíl uživatele (Broker)
- Rychle vyhodnotit tiket + projekt (riziko, parametry, podklady).
- Vybrat investora a spustit rezervaci.
- Mít přehled o kapacitě a časových limitech (expirace tiketu, 48h na podpis).

### Primární CTA
- **Vybrat investora** → **[BRK-220]** (z detailu i ze seznamu).

### URL / navigace
- Breadcrumb: **Tikety / Tiket #{ticket.public_id}**
- Detail je dostupný z kliknutí na kartu tiketu.
...
#### 4) Parametry tiketu
Tabulka / grid klíčových parametrů:
- Typ investice (Seniorní úvěr / Mezanin / Equity)
- Minimální výše rezervace
- Celkový objem
- Výnos (p.a.)
- Splatnost (měsíce)
- Úrokové období (měsíčně / kvartálně / na konci)

#### 5) Zajištění
- Pořadí (1. / 2. / …) + popis zajištění (zástava, ručení, notářský zápis…).

#### 6) LTV (Loan to Value)
- Vizualizace (kruh / gauge) + popisek.
- Pokud LTV není k dispozici, zobrazit „Není k dispozici“.

#### 7) Využití prostředků
- Textové shrnutí použití financování.
- Info box: „Uvedené využití prostředků je plánované a může se v průběhu realizace projektu měnit…“

#### 8) Podklady
- Seznam dokumentů (PDF/DOC/…):
  - název + velikost + typ souboru
  - akce: stáhnout / zobrazit
- „Ready Score“ (pokud existuje v datech).
- Poznámka: „Platforma podklady neověřuje…“ (audit / komunikace).
- U maskovaných tiketů mohou být některé podklady zamčené (disabled + tooltip „Odemkne se po podpisu…“).

#### 9) Doporučení investorů
- Sekce „Doporučení investorů“ = TOP investoři z matchingu.
- Každý řádek obsahuje:
  - identifikátor / jméno investora
  - krátký důvod (např. „Dlouhodobý zájem o Praha“)
  - CTA **Nabídnout** (spustí nabídku / otevře rezervaci předvyplněnou investorem)
- Volitelně CTA: „Zobrazit všechny shody“ → **[BRK-225] Matching drawer** (pokud používáme).

---

### Pravý sloupec (sticky aside)
...
- Primární CTA **Vybrat investora**
- Poznámka: „48h na podpis balíku dokumentů.“
...
## Stavové logiky (zkráceně)
- Pokud `ticket.status = filled/closed/expired` → CTA disabled.
- Pokud `masking_level != none`:
  - omezit citlivé informace a některé podklady,
  - zobrazit nápovědu (banner na listu + tooltipy na detailu).

---

## Analytika (min)
- `brk_ticket_detail_opened` (ticket_id)
- `brk_ticket_gallery_opened` (ticket_id)
- `brk_ticket_document_downloaded` (ticket_id, doc_type)
- `brk_ticket_select_investor_clicked` (ticket_id)
- `brk_ticket_offer_sent` (ticket_id, investor_id)

## [BRK-225] Matching investorů — Drawer (volitelně)

> Tento drawer je volitelný „power feature“ pro brokery. Pokud je v UI pouze sekce **Doporučení investorů**, lze drawer použít jako „Zobrazit všechny shody“.

### Cíl uživatele
- Vidět, kteří investoři (z portfolia brokera) mají nejvyšší šanci investovat do daného tiketu.
- Rychle vybrat investora a pokračovat do rezervace.

### Kde se otevře
- Z detailu tiketu: CTA „Zobrazit všechny shody“ (nebo podobné).
- Volitelně i ze seznamu tiketů (karta → „Matching“).

### Obsah draweru
- Nadpis: **Matching investorů**
- Subtext: „Na základě historie a preferencí by tento tiket mohl zajímat následující investory.“
- Souhrn:
  - počet „Vysoká shoda / Střední / Nízká“
  - možnost filtrovat dle úrovně shody

#### Ovládání
- Search: „Hledat investora…“
- Filter (dropdown): Vysoká / Střední / Nízká / Vše
- Toggle (volitelně): „Zahrnout neaktivní investory“ (default OFF)
- Toggle (volitelně): „Zahrnout nové investory“ (default ON)

#### Seznam investorů (rows)
Každý investor řádek obsahuje:
- Avatar / inic.
- Jméno / název investora
- Kontaktní e-mail (pokud relevantní)
- Tag / důvod (např. „Dlouhodobý zájem o Praha“, „Investuje 5–10 mil.“, „Preferuje 24 měsíců“)
- Badge shody (High/Medium/Low)
- CTA: **Vybrat** (primární)
  - po kliku:
    - buď otevře **[BRK-220]** s předvyplněným investorem,
    - nebo přímo přejde na **[BRK-230]** (pokud už máme potvrzení a text poznámky).

### Akce draweru (footer)
- **Zavřít**
- **Pokračovat** (pokud výběr investora probíhá v draweru; jinak není potřeba)

### Analytika (min)
- `brk_matching_drawer_opened` (ticket_id)
- `brk_matching_filter_changed` (ticket_id, filter)
- `brk_matching_investor_selected` (ticket_id, investor_id)
